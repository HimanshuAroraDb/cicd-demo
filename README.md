...
```
dbx deploy --jobs=cicd-demo-sample-integration-test --files-only
dbx launch --job=cicd-demo-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `cicd_demo/__init__.py` file and tag the current code version:
```
